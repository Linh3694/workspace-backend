...
// Đảm bảo thư mục `/uploads/Classes` tồn tại
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    if (file.fieldname === "zipFile") {
      ...
      // Ảnh đơn lẻ sẽ được lưu vào thư mục Classes
      cb(null, uploadPath);
    }
  },
  filename: (req, file, cb) => {
    if (file.fieldname === "zipFile") {
      ...
      cb(null, `class-images-${timestamp}.zip`);
    } else {
      // Ảnh đơn lẻ
      ...
      cb(null, `class-${timestamp}-${name}${ext}`);
    }
  },
});
...
const fileFilter = (req, file, cb) => {
  if (file.fieldname === "zipFile") {
    ...
    if (ext === ".zip") {
      ...
      cb(new Error("File phải có định dạng .zip"), false);
    }
  } else {
    // Kiểm tra file ảnh
