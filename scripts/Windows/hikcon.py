...
class AcsEventRequester:
    ...
    def _get_default_times(self):
        ...
        # Mặc định sync từ 1 ngày trước đến hiện tại
    ...
    def make_request(self, search_result_position=0, max_results=24, major=0, minor=0):
        ...
        while True:
            ...
            if "AcsEvent" not in response_data:
                print(f"Khong co du lieu AcsEvent tu {self.credentials['DEVICE_IP']}")
            ...
            if "InfoList" in response_data["AcsEvent"]:
                ...
                print(f"Khong co InfoList trong AcsEvent tu {self.credentials['DEVICE_IP']}")
    ...
    def upload_attendance(self, attendance_data):
        ...
        if not attendance_data:
            print("Khong co du lieu cham cong de upload")
        ...
        for i in range(0, len(attendance_data), 100):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"[OK] Batch {i//100 + 1} uploaded successfully: {result.get('message', 'Success')}")
                else:
                    print(f"[ERROR] Failed to upload batch {i // 100 + 1}: Status {response.status_code}")
                    print(f"Response: {response.text}")
            except requests.exceptions.RequestException as e:
                print(f"[ERROR] Network error uploading batch {i // 100 + 1}: {str(e)}")
...
if __name__ == "__main__":
    # Sử dụng đường dẫn tương đối, tương thích với cả Windows và Mac/Linux
    ...
    for credentials_file in credentials_files:
        try:
            ...
            if not os.path.exists(file_path):
                print(f"[WARNING] File khong ton tai: {credentials_file}")
                continue
                
            print(f"\n[SYNC] Bat dau sync tu {credentials_file}")
            requester = AcsEventRequester(file_path)
            attendance_data = requester.make_request()
            
            if attendance_data:
                print(f"[INFO] Tim thay {len(attendance_data)} records cham cong")
                upload_response = requester.upload_attendance(attendance_data)
                print(f"[OK] {upload_response}")
            else:
                print("[INFO] Khong co du lieu cham cong moi")
                
        except Exception as e:
            print(f"[ERROR] Loi khi sync voi {credentials_file}: {str(e)}")
            import traceback
            traceback.print_exc() 