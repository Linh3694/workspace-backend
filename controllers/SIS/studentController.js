...
exports.bulkUploadStudentImages = async (req, res) => {
  try {
    if (!req.file) {
    ...
    for (const entry of zipEntries) {
      try {
        ...
        // Tên file chỉ cần mã học sinh: studentCode.ext
        // Ví dụ: HS001.jpg, HS002.png
        ...
        const fs = require('fs');
        const uploadDir = `${__dirname}/../../uploads/Students`;
        if (!fs.existsSync(uploadDir)) {
          fs.mkdirSync(uploadDir, { recursive: true });
        }

        zip.extractEntryTo(entry, uploadDir, false, true, newFileName);
        ...
        if (existingPhoto) {
          ...
          existingPhoto.updatedAt = Date.now();
          await existingPhoto.save();
          results.success.push(`${studentCode}: Cập nhật ảnh thành công`);
        } else {
          // Tạo photo mới
          const photoData = {
            student: student._id,
          ...
          await Photo.create(photoData);
          results.success.push(`${studentCode}: Upload ảnh thành công`);
        }
