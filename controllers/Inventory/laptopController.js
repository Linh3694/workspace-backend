...
// Lấy danh sách laptop với pagination
exports.getLaptops = async (req, res) => {
  try {
    ...
    console.log(`[DB] Fetching laptops page ${page} from database`);
...
exports.getLaptopFilterOptions = async (req, res) => {
  try {
    console.log('[Filter Options] Fetching laptop filter options');
    
    // Aggregate data from all laptops
    const aggregationPipeline = [
      {
        $lookup: {
          from: 'users',
    ...
    console.error('Error fetching laptop filter options:', error);
    res.status(500).json({
